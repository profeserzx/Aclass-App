...
import { getStkPushStatusAction } from "@/app/actions/mpesa";
...
export default function MpesaStatusPoller({ requestId }: { requestId: string }) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      const result = await getStkPushStatusAction(requestId);
      ...
      if (result.status !== "pending") {
        ...
        router.refresh(); // pick up the updated fee balance / payment history
        return;
      }
      // Stop after ~2 minutes (Safaricom's own prompt times out around then too).
      if (attempts < 40) {
